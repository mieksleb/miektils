...
class Input:
    def __init__(self, backend="CUDA", steps="1e3", diff="2.50", thermostat="john", temp="300K", dt="0.005", salt_conc="0.5", top_file="generate.top", conf_file="generate.dat", last_conf_file="last_conf.dat", traj_file="trajectory.dat", energy_file="energy.dat", force_file="forces.dat"):
...
chinput = Input(backend="CUDA", steps="1e7", diff=2.50, thermostat="john", top_file = sys.argv[1], conf_file = sys.argv[2], traj_file = sys.argv[3])   
